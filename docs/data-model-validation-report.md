...
프로젝트의 최종 목표는 행정동별 미래 택시 수요를 지도와 시간대별 그래프로 설명하는 것입니다.
...
아직 불가: 실제 택시 호출량 절대 예측, 기사 공급 최적화, 운영 지시
```
...
행정동별 1시간 뒤 수요 score를 지도/그래프 입력으로 생성할 수 있다
...
그 결과를 지도 heatmap과 시간대별 수요 그래프의 입력으로 사용하는 구조입니다.
```
