...
const LOG_PATH = "data/processed/live_validation/traffic_forecast_log.jsonl";
const PROCESSED_OUTPUT = "data/processed/live_validation/traffic_forecast_comparison.json";
const PUBLIC_OUTPUT = "public/traffic-forecast-comparison.json";
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        files.push({
          ...
          mtime_ms: (await stat(entryPath)).mtimeMs,
        });
      }
    }
  }
...
function compareForecastToTraffic(forecast, trafficSnapshot) {
  ...
  const rows = predictedRows
    .map((predicted) => {
      ...
      return {
        ...
        predicted_traffic_volume_proxy: numberOrNull(predicted.predicted_traffic_volume_proxy),
        ...
        speed_error_kmh:
          predictedSpeed == null || actualSpeed == null ? null : round(predictedSpeed - actualSpeed, 2),
        abs_speed_error_kmh:
  ...
  const predictedCongestionRanks = rankRows(rows, "predicted_congestion_score");
  const actualCongestionRanks = rankRows(rows, "actual_congestion_score");
  const predictedSpeedRanks = rankRows(rows, "predicted_avg_speed_kmh", "asc");
  const actualSpeedRanks = rankRows(rows, "actual_avg_speed_kmh", "asc");
  const rankedRows = rows
    .map((row) => ({
      ...row,
      predicted_congestion_rank: predictedCongestionRanks.get(row.dong_name) ?? null,
      actual_congestion_rank: actualCongestionRanks.get(row.dong_name) ?? null,
      predicted_slowest_rank: predictedSpeedRanks.get(row.dong_name) ?? null,
      actual_slowest_rank: actualSpeedRanks.get(row.dong_name) ?? null,
    }))
    .sort(
      (left, right) =>
        (left.predicted_congestion_rank ?? 999) - (right.predicted_congestion_rank ?? 999),
    );
  ...
  const absSpeed = rankedRows
    ...
    .filter((value) => Number.isFinite(value));
  const topPredicted = rankedRows.find((row) => row.predicted_congestion_rank === 1) ?? null;
  const topActual = rankedRows.find((row) => row.actual_congestion_rank === 1) ?? null;
  ...
  return {
    ...
    comparison_type: "predicted_road_congestion_vs_observed_citydata_road_congestion",
    note:
      "Predicted congestion/speed are live-calibrated estimates. Actual values are Seoul citydata dong-level road observations collected at/after the target time.",
    overall: {
      ...
      congestion_rank_spearman: round(
        pearson(
          rankedRows.map((row) => row.predicted_congestion_rank),
      ...
      top_predicted_congestion_dong: topPredicted?.dong_name ?? null,
      top_actual_congestion_dong: topActual?.dong_name ?? null,
      same_top_congestion_dong:
        topPredicted?.dong_name != null && topPredicted?.dong_name === topActual?.dong_name,
    },
    rows: rankedRows,
  };
}
...
for (const forecast of forecasts) {
  ...
  if (matchedSnapshot) {
    completed.push(compareForecastToTraffic(forecast, matchedSnapshot));
  } else {
    waiting.push({
      generated_at: forecast.generated_at ?? null,
      target_datetime: forecast.target_datetime ?? null,
      feature_datetime: forecast.feature_datetime ?? null,
      top_predicted_congestion_dong: forecast.regions?.[0]?.dong_name ?? null,
      top_predicted_congestion_score: forecast.regions?.[0]?.predicted_congestion_score ?? null,
