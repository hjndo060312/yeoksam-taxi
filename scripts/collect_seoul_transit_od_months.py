#!/usr/bin/env python3
"""Collect and process Seoul dong-level transit OD ZIP files (OA-21226).

This is a Python-native replacement for `scripts/collect-seoul-transit-od.mjs`.
It avoids Node child-process spawning, which can be restricted in some sandboxes.

Flow:
1) Scrape the datasetView page to find monthly ZIP seq ids.
2) Download each ZIP (cached under data/raw/transit_od/).
3) Process it via `scripts/process_seoul_transit_od_zip.py`.
4) Optionally delete the raw ZIP to keep the repo small.
5) Write a status JSON summary under data/processed/transit_od/.
"""
...
DATASET_VIEW_URL = "https://data.seoul.go.kr/dataList/OA-21226/F/1/datasetView.do"
DOWNLOAD_URL = "https://datafile.seoul.go.kr/bigfile/iot/inf/nio_download.do?&useCache=false"
INF_ID = "OA-21226"
INF_SEQ = "1"
FILE_PATTERN = re.compile(
    r'title="(tpss_emd_odh_(\d{6})\.zip)"\s+onclick="javascript:downloadFile\(\'([^\']+)\'\);?"'
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--raw-dir",
        default="data/raw/transit_od",
        help="Raw ZIP cache directory",
    )
    parser.add_argument(
        "--out-dir",
        default="data/processed/transit_od",
...
def download_zip(raw_dir: Path, meta: FileMeta) -> Path:
    ...
    body = urlencode({"infId": INF_ID, "seq": meta.seq, "infSeq": INF_SEQ}).encode("utf-8")
    req = Request(
        DOWNLOAD_URL,
        method="POST",
        data=body,
...
def process_zip(period: str, zip_path: Path, out_dir: Path) -> None:
    processor = Path("scripts/process_seoul_transit_od_zip.py")
    cmd = [
        sys.executable,
        str(processor),
        "--period",
...
def main() -> None:
    ...
    status = {
        "source": "Seoul Open Data Plaza OA-21226 (Seoul transit OD)",
    ...
    status_path = out_dir / f"seoul_transit_od_collection_{start_period}_{end_period}.status.json"
    status_path.write_text(json.dumps(status, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(f"Wrote {status_path.as_posix()}")


if __name__ == "__main__":
    # Avoid inheriting any proxy settings accidentally.
